...
!!! danger "`docker-compose` versus `docker compose`"
    ...
    As [mentioned by Docker](https://docs.docker.com/compose/), from July 2023 Compose V1 *(`docker-compose`)* will not receive updates anymore which makes it deprecated and replaced by Compose V2 *(`docker compose`)* directly embedded into the `docker` command line.

Whether you choose to install [Docker](https://www.docker.com/) or [Podman](https://podman.io/) and their `composer` on [Linux](https://en.wikipedia.org/wiki/Linux), [Mac OS](https://en.wikipedia.org/wiki/Mac_operating_systems) or [Windows](https://en.wikipedia.org/wiki/Microsoft_Windows), please refer to the official documentation[^1].
...
You should not run the `docker` command as `root` user or using the `sudo` command. If so then you will get some error messages such as `Permission denied` and some containers could restart in a loop.
...
Once added to the `docker` group you will have to log out from the current session _(graphical or [SSH](https://en.wikipedia.org/wiki/Secure_Shell))_ in order to get the group added to your user once you reconnect.

Once reconnected, run the following command to ensure the `docker` group has been appended to your user.
...
_The `GID` could differ compared to your setup._
