...
All audio playback *(sounds played on the speakers)* is handled by this service by leveraging the sound server/service from the system where it's running.
...
## :material-console: ovos-cli

Command-line client container for interacting with OVOS. It exposes tools such as
`ovos-cli-client`, `ovos-config`, and `ovos-speak` for messagebus interaction and
configuration management.

## :material-brain: ovos-core

The skills service is responsible for loading skills and [intent](./terms.md#intent) parsers, and handles all user queries.
...
Base implementations for [Natural language processing](https://en.wikipedia.org/wiki/Natural_language_processing) *(NLP)* tasks, from baseline heuristics to classic machine learning models that run everywhere.
...
!!! warning "Component dependency"
    ...
    This component depends on [ovos-gui-websocket](./components.md#ovos-gui-websocket).

## :material-message-processing: ovos-gui-websocket

GUI websocket service that manages GUI state and implements the GUI protocol.
...
!!! tip "PHAL admin variant"
    ...
    In addition to the initial PHAL service, PHAL admin is running as `root` or as a privileged user. This variant allows PHAL admin to perform actions which require more privileges such as `reboot` for example.

## :material-connection: ovos-plugin-manager

OPM is the OVOS Plugin Manager, this base package provides arbitrary plugins to the OVOS ecosystem. OPM plugins import their base classes from OPM making them portable and independent of `ovos-core`, plugins can be used in standalone projects.
...
A lightweight, dead-simple intent parser which aims to replace the current intent parser; [Padatious](https://mycroft-ai.gitbook.io/docs/mycroft-technologies/padatious).
