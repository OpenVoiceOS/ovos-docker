...
if [[ -f "$skills_list" ]]; then
    if ! diff -q -B <(grep -vE '^\s*(#|$)' "$skills_list") <(grep -vE '^\s*(#|$)' "$skills_list_state" 2>/dev/null) &>/dev/null; then
        ...
        uv pip install -r "$skills_list" -c "https://raw.githubusercontent.com/OpenVoiceOS/ovos-releases/${OVOS_RELEASES_REF:-main}/constraints-${OVOS_CHANNEL}.txt"
...
for skill_dir in */; do
    ...
    if [[ -f "requirements.txt" ]]; then
        ...
        uv pip install -r requirements.txt -c "https://raw.githubusercontent.com/OpenVoiceOS/ovos-releases/${OVOS_RELEASES_REF:-main}/constraints-${OVOS_CHANNEL}.txt"
    fi
